...
export async function fetchChapter(options: FetchChapterParams) {
    let { input, verseData, verseLimit, verseReference, selectedVerseIndex, selectedTranslation } = options;
    
    const osis = parseQuery(input)!;
    ...
    verseReference = resolved.verses.length === 0 ? verseReference : getVerseReference(verseData, osis, selectedVerseIndex);
...
export function getVerseReference(verseData: Verse[], osis: OSISReference, selectedVerseIndex: number): string {

...
<Modal modalTitle="Export Verse Set" bind:showModal={showExportModal}>
     ...
     {#if selectedIndex === 0}
          ...
          <input bind:value={email} class="w-full h-11 px-3 mb-2 bg-light_grey/8 border border-border_accent rounded outline-none"/>

          <FooterButton action="Send" disabledCondition={!validEmail} eventHandler={() => {}} />
     {:else}
          <div class="flex flex-row items-center px-3 mb-2 gap-2 w-full h-13 border border-border_accent rounded">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="text-accent_btn">
